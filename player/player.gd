...
var wallrun_gravity_scale: float = 0.25
var traction: float = 15.0
var air_traction: float = 1.0
var speed: float = 6.0
...
#stores the walls normal
	...
	if grapple_point != Vector3.INF:
		...
		global_position = grapple_point + (global_position - grapple_point).limit_length(
				cursor.target_position.length())
...
func _input(event: InputEvent) -> void:
	...
	if event.is_action_pressed(&"dagger") and counters["jumps"] > 0:
...
func tick_counter(counter: String) -> void:
	...
	if 0 in [counters["jumps"], counters["health"]]:
